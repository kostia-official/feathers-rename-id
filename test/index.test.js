...
test('id', async (t) => {
  ...
  const { body: found } = await supertest.get('/posts');
  const { body: paginatedFound } = await supertest.get('/paginated-posts');
  ...
  t.is(paginatedFound.data[0].id, post.id);
...
test('params', async (t) => {
  ...
  const { body: paginatedFound } = await supertest.get('/paginated-posts').query({ id: post.id }).send({ name: 'new' });
